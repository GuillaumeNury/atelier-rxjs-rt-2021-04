...
export class Case2Component {
  ...
  constructor() {
    this._originalTitle$ = new BehaviorSubject('count-subscription-reloaded-case2');
    this._suffix$ = new BehaviorSubject('🚀');

    this.originalTitle$ = this._originalTitle$.pipe(rxDebug('originalTitle$'))
    ...
    this.titleLength$ = this.title$.pipe(map(title => title.length), rxDebug('titleLength$'));
    this.twiceTitleLength$ = this.titleLength$.pipe(map(titleLength => titleLength * 2), rxDebug('twiceTitleLength$'));
  }
